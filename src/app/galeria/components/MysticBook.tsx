...
const BASE =
  "https://rp9jryczlxa748zk.public.blob.vercel-storage.com/conmemoracion_jx_edicion_digital_png/";
const PW = 420;
const PH = 670;
...
const Page = React.forwardRef<
  HTMLDivElement,
  { image: string | null; index: number; isCover?: boolean }
>((props, ref) => {
  return (
    <div
      className={`page bg-[#0a0908] overflow-hidden relative ${props.isCover ? "hard" : ""}`}
      ref={ref}
    >
      {props.image ? (
        <NextImage
...
export function MysticBook() {
  ...
  const [bookState, setBookState] = useState<"cover" | "open" | "back">(
    "cover",
  );
    ...
    if (isFullscreen) {
      ...
      if (scrollPosRef.current !== null) {
        ...
        requestAnimationFrame(() => {
          window.scrollTo({
            top: scrollPosRef.current as number,
            behavior: "instant",
          });
  ...
  const getTransformClass = () => {
    ...
    if (bookState === "cover")
      return "max-[872px]:translate-x-0 min-[872px]:-translate-x-[210px]";
    if (bookState === "back")
      return "max-[872px]:translate-x-0 min-[872px]:translate-x-[210px]";
    return "translate-x-0";
  };

  const displayedPages = isMobileScreen
    ? PAGES.filter((_, i) => i !== 1)
    : PAGES;
  ...
  return (
    <section
      ...
      className={`w-full flex flex-col items-center justify-center overflow-hidden ${
        isFullscreen
          ? "fixed inset-0 z-[150] h-[100dvh] bg-[#0A0908] py-4 px-2 overflow-auto"
          : "relative py-20 md:py-32 px-4"
      }`}
    ...
    >
      ...
      <div
        className={`relative z-10 text-center max-w-2xl ${isFullscreen ? "hidden" : "mb-12"}`}
      >
      ...
      {!ready || !mounted ? (
        <div
          className="relative z-10 mx-auto flex justify-center w-full overflow-hidden"
          style={{ maxWidth: PW }}
        >
          <div
            className="w-full bg-[#111] border border-white/5 shadow-2xl flex flex-col items-center justify-center gap-6 animate-pulse relative"
      ...
      <div
        className={`flex items-center justify-center ${isFullscreen ? "relative z-[150] w-auto bg-[#0A0908]/90 px-3 py-3 md:px-4 md:py-3 rounded-full border border-[#E8E2D2]/10 shadow-[0_0_40px_rgba(0,0,0,0.8)] backdrop-blur-xl gap-6" : "gap-6 relative z-10 mt-6"}`}
      >
        <button
        ...
        >
          <ChevronLeft
            size={isFullscreen ? 24 : 15}
            className="group-hover:-translate-x-0.5 transition-transform"
          />
          {!isFullscreen && (
            <span className="font-mono text-[11px] tracking-widest uppercase hidden md:inline">
              Anterior
            </span>
          )}
        </button>
